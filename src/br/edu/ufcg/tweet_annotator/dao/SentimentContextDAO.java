...
public class SentimentContextDAO extends GenericDAO {
	
	public SentimentContextDAO() {	}
	
	public SentimentContext getContextById(Integer contextId) {
		
		SentimentContext result = null;
		
		String query = "SELECT * FROM sentiment_contexts WHERE context_id = ? ";
		...
		try {
			...
			if (res.next()) {
				result = new SentimentContext(contextId);
	...
	public List<SentimentContext> loadContexts() {
		
		List<SentimentContext> result = new ArrayList<SentimentContext>();
		
		String query = "SELECT * FROM sentiment_contexts ORDER BY name ASC";
		...
		try {
			...
			while (res.next()) {
				SentimentContext sc = new SentimentContext(res.getInt("context_id"));
