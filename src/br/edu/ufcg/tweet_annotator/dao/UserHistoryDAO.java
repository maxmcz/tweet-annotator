...
public class UserHistoryDAO extends GenericDAO {
	
	public UserHistoryDAO() {  }

	public void doLogout(Integer userId, String ip) {
	...
	public Integer persistNew(UserHistory history) {
		
		Integer resultId = null;
		
		String query = "INSERT INTO user_access_history(user_id, datetime, ip_address, type) " +
					   " VALUES (?, NOW(), ?, ?) ";
		
		try {
			PreparedStatement stmt = getConnection().prepareStatement(query);
			stmt.setInt(1, history.getUserId());
			stmt.setString(2, history.getIp());
