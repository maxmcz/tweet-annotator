...
public class SemanticContextDAO extends GenericDAO {
	
	public SemanticContextDAO() {	}
	
	public SemanticContext getContextById(Integer contextId) {
		
		SemanticContext result = null;
		
		String query = "SELECT * FROM semantic_contexts WHERE context_id = ? ";
		...
		try {
			...
			if (res.next()) {
				result = new SemanticContext(contextId);
	...
	public List<SemanticContext> loadContexts() {
		
		List<SemanticContext> result = new ArrayList<SemanticContext>();
		
		String query = "SELECT * FROM semantic_contexts ORDER BY \"order\" ASC";
		...
		try {
			...
			while (res.next()) {
				SemanticContext sc = new SemanticContext(res.getInt("context_id"));
